...
In order to calculate power I needed to estimate the standard deviation of R~50~. Unfortunately, the confidence intervals were not symmetric after translating them to be in terms of range. To be conservative in the power calculations I choose the largest margin to determine the standard deviation.

```{r}
R50_dat %>%
  mutate(across(c(lower, upper), ~ .x - R50, .names = "{col}_margin"),
         turbines = c("None", "Present")) %>%
  mutate(lower_margin = lower_margin %>% 
...
If we assume there are two standard deviations from R~50~ to the interval bounds, then the largest the standard deviation could be is about 2. Since the difference in R~50~ is about 5 data miles less under conditions when there are wind turbines verses no turbines, then the R~50s~ are about 2.5 standard deviation from each other. In other words, the effect size is about 2.5 data miles. Using this information, I created the plot below.
